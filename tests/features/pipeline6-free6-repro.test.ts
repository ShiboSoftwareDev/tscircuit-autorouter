...
test("pipeline6 free-6 extracted node recovers with projection fallback", () => {
  const polygon = [
  ...
  expect(solver.successfulProjectionFallbackLabel).not.toBe("initial")
  expect(solver.solvedNodeWithPortPoints?.portPoints).toBe(
    solver.params.nodeWithPortPoints.portPoints,
  )
})
